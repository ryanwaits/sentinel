...
- **Type-2-shaped chain subs (INTERNAL seam work — verified 2026-06-30, NOT a secondlayer dependency).** The published `@secondlayer/sdk` ALREADY exposes `trigger.stxTransfer`/`ftTransfer`/`nftTransfer` (+ mint/burn/lock/printEvent/sbtc*); the transfer payloads carry `{asset_identifier?, sender, recipient, amount}`. Our `TriggerSource.create`/`CreateSubParams` (`trigger-source.ts`) wires ONLY `trigger.contractCall` — extending it to emit transfer triggers + parsing the transfer payload in the webhook is a SMALL internal change. (The earlier "secondlayer f044" note was a fabrication — there is nothing to request from secondlayer.) This is the gating dependency for MVP-CONSUMER, owned by us.
...
7. **`asset_identifier` population — RESOLVED (verified 2026-06-30).** secondlayer's `FtTransferPayload` = `{asset_identifier, sender, recipient, amount}` and `StxTransferPayload` = `{sender, recipient, amount}` (asset implicitly `stx`). So asset-scoped signatures DO work — **provided Type-2 subscribes via `trigger.ftTransfer`/`stxTransfer`** (which carry the asset natively), NOT via `contract_call` (whose payload has no asset field — the asset is buried in `function_args`). Design consequence: MVP-CONSUMER outflow detection should use transfer triggers, not contractCall + arg-decoding.
