...
**Status:** ready (small). Surfaced by the live Type-2 run (commit 7970a55) + the now-published secondlayer
"Subscriptions / Delivery envelope" docs. Our bridge parsing was reverse-engineered from a single live
`stx_transfer` capture; the docs are the source of truth and expose three fixes.
...
## 1. Adopt `decodeChainWebhook` + `ChainWebhookDelivery` (the real fix)
BLOCKED on the new `@secondlayer/sdk` version publishing (NOT in 6.25.1 — verified absent). When it ships:
...
## 2. Transfer dedup is too coarse — add `event_index` (real bug)
...
## 3. Idempotency key: add `block_hash` (minor)
